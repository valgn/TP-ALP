...
stepComm (Let var exp) s = 
  case evalExp exp s of 
    Right (newVal :!: expState) -> let newState = update var newVal expState in Right (Skip :!: newState)
    Left e -> Left e
...
stepComm (RepeatUntil c expB) s = Right ((Seq c (IfThenElse expB Skip (RepeatUntil c expB))) :!: s)
